...
namespace YoungRebel.Scriptes.Player
{
    public class WallRunTutorial : MonoBehaviour
    {
        ...
        [Header("Wall Running")] public LayerMask whatIsWall;
        ...
        [SerializeField] private LayerMask climbingDetectionLayer;
        [SerializeField] private Transform checkGroundForClimbingTransform;
        ...
        void Start()
        {
            ...
            climbingController = new ClimbingController(rb, transform);
        }
        ...
        private void WallRunInput() //make sure to call in void Update
        {
            ...
            {
                //Wallrun
                if (Input.GetKey(KeyCode.W) && isWallRight) StartWallrun();
        ...
        private void StartWallrun()
        {
            ...
            if (rb.velocity.magnitude <= maxWallSpeed)
            {
                ...
                if (isWallRight)
                {
                    ...
                    rb.AddForce(-orientation.up * wallrunForce / 8 * Time.deltaTime);
                }
                else
                {
                    rb.AddForce(-orientation.right * wallrunForce / 5 * Time.deltaTime);
        ...
        {
            if (rb.velocity.x <= 0f) isWallRunning = false;
            if (rb.velocity.y <= 0f) isWallRunning = false;
            isWallRunning = false;
            rb.useGravity = true;
        }
        ...
        private void MyInput()
        {
            ...
            //Climbing
            if (Physics.Raycast(orientation.position, orientation.forward, 1, climbingDetectionLayer) || climbingController.IsClimbing())
            {
                 ClimbProcess();
            }
            else
        ...
        private void StartCrouch()
        {
            ...
            if (rb.velocity.magnitude > 0.5f)
            {
                if (grounded)
                {
                    rb.AddForce(orientation.transform.forward * 0.01f);
                }
            }
        }
        ...
        private void Jump()
        {
            ...
            if (isWallRunning)
            {
                ...
                //normal jump
                if (isWallLeft && !Input.GetKey(KeyCode.W) || isWallRight && !Input.GetKey(KeyCode.W))
                {
                ...
                //sidwards wallhop
                if (isWallRight || isWallLeft && Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)) rb.AddForce(-orientation.up * jumpForce * 2f);
        ...
        private void ClimbProcess()
        {
            ...
            {
                if (Physics.Raycast(checkGroundForClimbingTransform.position, Vector3.down, out RaycastHit raycastHit, CheckDistance, whatIsGround))
                {
                    climbingController.StartClimb(raycastHit.transform);
                }
            }

            if (climbingController.IsClimbing())
            {
                ClimbingParameters climbingParameters = new ClimbingParameters
                {
                    ClimbForce = climbForce,
                    MaxClimbSpeed = maxClimbSpeed,
                    Orientation = orientation,
                };
                
                climbingController.Climb(climbingParameters);
            }

            if (Input.GetKey(KeyCode.S))
            {
                climbingController.FinishClimb();
            }
        }
        ...
        private void CounterMovement(float x, float y, Vector2 mag)
        {
            ...
            if (crouching)
            {
                rb.AddForce(moveSpeed * Time.deltaTime * -rb.velocity.normalized * slideCounterMovement * 0.2f);
                return;
            }
